...
interface NovaPoshtaCityAutocompleteProps {
  value: string;
...
export function NovaPoshtaCityAutocomplete({ 
  value, 
  onChange, 
  placeholder = "Оберіть місто...",
  className 
}: NovaPoshtaCityAutocompleteProps) {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      setLoading(true);
      try {
        const { data, error } = await supabase.functions.invoke('nova-poshta-cities', {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] max-w-[95vw] p-0" align="start">
        <Command>
          ...
          />
          <CommandEmpty>
            {loading ? "Завантаження..." : "Міста не знайдено"}
          </CommandEmpty>
            ...
            {cities.map((city) => (
              ...
              >
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <span className="text-sm text-muted-foreground truncate">{city.area}</span>
                </div>
              </CommandItem>
            ))}
          </CommandGroup>
